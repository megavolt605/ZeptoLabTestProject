...
@interface ZLStoneShapeSmall ()

@end

@implementation ZLStoneShapeSmall

#pragma mark Initialization

- (id) init {
    if (self = [super initWithCount: 13 edgeCount: 8]) {
        self.shape[ 0] = ZLVectorMakeWithPoint(  0.0, -1.4);
        self.shape[ 1] = ZLVectorMakeWithPoint(  0.0,  0.0);
        self.shape[ 2] = ZLVectorMakeWithPoint(  1.0, -1.0);
        self.shape[ 3] = ZLVectorMakeWithPoint(  1.4,  0.0);
        self.shape[ 4] = ZLVectorMakeWithPoint(  0.0,  0.0);
        self.shape[ 5] = ZLVectorMakeWithPoint(  1.0,  1.0);
        self.shape[ 6] = ZLVectorMakeWithPoint(  0.0,  1.4);
        self.shape[ 7] = ZLVectorMakeWithPoint(  0.0,  0.0);
        self.shape[ 8] = ZLVectorMakeWithPoint( -1.0,  1.0);
        self.shape[ 9] = ZLVectorMakeWithPoint( -1.4,  0.0);
        self.shape[10] = ZLVectorMakeWithPoint(  0.0,  0.0);
        self.shape[11] = ZLVectorMakeWithPoint( -1.0, -1.0);
        self.shape[12] = ZLVectorMakeWithPoint(  0.0, -1.4);
